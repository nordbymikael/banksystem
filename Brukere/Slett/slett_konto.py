def slett_konto (fødselsnummer):
    if count_kontonummere(fødselsnummer) == True: # sjekker først om det finnes kontoer som man kan slette
        print("\nHer er alle kontoene dine:")
        kontonummere_på_valgt_fødselsnummer(fødselsnummer) # printer ut kontonummere
        ...
        if konto_som_slettes.isdigit() == True and len(konto_som_slettes) == 11 and kontonummer_sjekk_om_finnes_på_fødselsnummer(konto_som_slettes, fødselsnummer) == True:
            mycursor.execute(f"SELECT SALDO FROM KONTOER WHERE KONTONUMMER = {konto_som_slettes}")
            ...
            if svar_bekreft == "Ja": # slett kontoen dersom du svarte "Ja"
                slett_historikk_og_konto(konto_som_slettes)
                print(f"\nDu har slettet kontoen med kontonummeret {konto_som_slettes}.\nHer er alle kontoene som er igjen på brukeren din:")
                kontonummere_på_valgt_fødselsnummer(fødselsnummer) # du får printet ut kontoer som du har igjen på brukeren
