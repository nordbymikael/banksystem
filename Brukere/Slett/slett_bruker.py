def slett_bruker (fødselsnummer):
    ...
    if svar_bekreft == "Ja": # dersom du svarer "Ja", så vil brukeren din bli slettet
        mycursor.execute(f"SELECT KONTONUMMER FROM KONTOER WHERE FØDSELSNUMMER = \"{fødselsnummer}\"") # spørring som henter kontonummere på valgt fødselsnummer
        myresult = mycursor.fetchall()
        
        for x in myresult:
            slett_historikk_og_konto(legge_til_nuller(x[0], 1)) # kaller funksjonen som sletter kontoen som for loopen går gjennom nå. legge til nuller funksjonen legger til nuller til kontonummeret som ble funnet
            print(f"All historikk og saldoen til kontoen {legge_til_nuller(x[0], 1)} ble slettet.") # sier at kontoen er slettet. legge til nuller funksjonen legger til nuller
        mycursor.execute(f"DELETE FROM KUNDEINFO WHERE FØDSELSNUMMER = {fødselsnummer}") # sletter informasjon fra KUNDEINFO som var relatert til alle kontonene som ble slettet
        mydb.commit() # lagre endringer
